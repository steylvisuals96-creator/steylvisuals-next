...
function Input({ label, value, onChange, multiline = false }: {
  ...
  const style: React.CSSProperties = {
    ...
    color: "#FDFAF7", fontSize: "0.875rem", outline: "none",
    fontFamily: "inherit", resize: "vertical", boxSizing: "border-box",
  };
  return (
    <div style={{ marginBottom: "1.25rem" }}>
      <label style={{ display: "block", fontSize: "0.7rem", color: "rgba(253,250,247,0.4)", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: "0.4rem" }}>{label}</label>
      {multiline
    ...
    return (
      <div style={{ minHeight: "100svh", backgroundColor: "#0A0A0A", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ width: "100%", maxWidth: "360px", padding: "2.5rem", border: "1px solid rgba(184,132,58,0.18)", borderRadius: "16px", backgroundColor: "#111" }}>
          ...
          <button onClick={handleLogin} style={{ width: "100%", marginTop: "1rem", padding: "0.85rem", backgroundColor: "#B8843A", border: "none", borderRadius: "8px", color: "#0A0A0A", fontSize: "0.9rem", fontWeight: 600, cursor: "pointer" }}>
            Inloggen
          </button>
        </div>
      </div>
    ...
    <div style={{ minHeight: "100svh", backgroundColor: "#0A0A0A", color: "#FDFAF7" }}>
      ...
      <div style={{ borderBottom: "1px solid rgba(255,255,255,0.07)", padding: "1.25rem 2rem", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          ...
          <span style={{ marginLeft: "0.75rem", fontSize: "0.7rem", color: "#B8843A", letterSpacing: "0.1em", textTransform: "uppercase" }}>Beheer</span>
        </div>
        <button onClick={() => { localStorage.removeItem("sv_admin_token"); setAuthed(false); setToken(""); }} style={{ fontSize: "0.75rem", color: "rgba(253,250,247,0.35)", background: "none", border: "none", cursor: "pointer" }}>
          Uitloggen
        </button>
      </div>
      ...
      <div style={{ display: "flex", height: "calc(100svh - 61px)" }}>
        ...
        <div style={{ width: "200px", borderRight: "1px solid rgba(255,255,255,0.07)", padding: "1.5rem 1rem", flexShrink: 0, display: "flex", flexDirection: "column", gap: "0.25rem" }}>
          {/* Content section */}
          <p style={{ fontSize: "0.65rem", color: "rgba(253,250,247,0.3)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "0.5rem", paddingLeft: "0.5rem" }}>Inhoud</p>
          <button
            onClick={() => setView("content")}
            style={{ display: "flex", alignItems: "center", gap: "0.6rem", width: "100%", padding: "0.6rem 0.75rem", borderRadius: "8px", border: "none", cursor: "pointer", textAlign: "left", fontSize: "0.85rem", backgroundColor: view === "content" ? "rgba(184,132,58,0.15)" : "transparent", color: view === "content" ? "#B8843A" : "rgba(253,250,247,0.6)" }}
          >
            <span>✏️</span> Teksten
          </button>

          {/* Media section */}
          <p style={{ fontSize: "0.65rem", color: "rgba(253,250,247,0.3)", letterSpacing: "0.1em", textTransform: "uppercase", margin: "1rem 0 0.5rem", paddingLeft: "0.5rem" }}>Media</p>
          {FOLDERS.map(f => (
            <button
              key={f.id}
              onClick={() => { setView("files"); setFolder(f.id); setProgress([]); }}
              style={{ display: "flex", alignItems: "center", gap: "0.6rem", width: "100%", padding: "0.6rem 0.75rem", borderRadius: "8px", border: "none", cursor: "pointer", textAlign: "left", fontSize: "0.85rem", backgroundColor: view === "files" && folder === f.id ? "rgba(184,132,58,0.15)" : "transparent", color: view === "files" && folder === f.id ? "#B8843A" : "rgba(253,250,247,0.6)" }}
            >
          ...
          {view === "content" && (
            <div style={{ maxWidth: "680px" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "2rem" }}>
                <h2 style={{ fontFamily: "serif", fontSize: "1.4rem", fontWeight: 400, color: "#FDFAF7" }}>Teksten & inhoud</h2>
                <button
                  onClick={saveContent}
                  style={{ padding: "0.65rem 1.5rem", backgroundColor: saved ? "#4ade80" : "#B8843A", border: "none", borderRadius: "8px", color: "#0A0A0A", fontSize: "0.85rem", fontWeight: 600, cursor: "pointer", transition: "background 0.3s" }}
                >
              ...
              <section style={{ marginBottom: "2.5rem" }}>
                <p style={{ fontSize: "0.7rem", color: "#B8843A", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "1rem", borderBottom: "1px solid rgba(184,132,58,0.15)", paddingBottom: "0.5rem" }}>Contactgegevens</p>
              ...
              {/* Hero */}
              <section style={{ marginBottom: "2.5rem" }}>
                <p style={{ fontSize: "0.7rem", color: "#B8843A", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "1rem", borderBottom: "1px solid rgba(184,132,58,0.15)", paddingBottom: "0.5rem" }}>Homepagina</p>
                <Input label="Ondertitel hero" value={content.hero.subtext} onChange={v => setContent(c => ({ ...c, hero: { ...c.hero, subtext: v } }))} multiline />
              </section>

              {/* Stats */}
              <section style={{ marginBottom: "2.5rem" }}>
                <p style={{ fontSize: "0.7rem", color: "#B8843A", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "1rem", borderBottom: "1px solid rgba(184,132,58,0.15)", paddingBottom: "0.5rem" }}>Statistieken</p>
              ...
              {/* Vastgoed */}
              <section style={{ marginBottom: "2.5rem" }}>
                <p style={{ fontSize: "0.7rem", color: "#B8843A", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "1rem", borderBottom: "1px solid rgba(184,132,58,0.15)", paddingBottom: "0.5rem" }}>Vastgoed marketing pagina</p>
          ...
          {view === "files" && (
            <>
              <div
                ...
                style={{ border: `2px dashed ${dragging ? "#B8843A" : "rgba(184,132,58,0.25)"}`, borderRadius: "12px", padding: "2.5rem", textAlign: "center", cursor: "pointer", backgroundColor: dragging ? "rgba(184,132,58,0.04)" : "transparent", transition: "all 0.2s", marginBottom: "2rem" }}
              >
                <p style={{ fontSize: "2rem", marginBottom: "0.5rem" }}>+</p>
                <p style={{ color: "rgba(253,250,247,0.5)", fontSize: "0.9rem" }}>Sleep bestanden hierheen of klik om te uploaden</p>
                <p style={{ color: "#B8843A", fontSize: "0.75rem", marginTop: "0.35rem" }}>→ {FOLDERS.find(f => f.id === folder)?.label}</p>
                    ...
                    <div key={file.key} style={{ backgroundColor: "#111", borderRadius: "10px", overflow: "hidden", border: "1px solid rgba(255,255,255,0.07)" }}>
                      ...
                      <div style={{ padding: "0.75rem" }}>
                        ...
                        <div style={{ display: "flex", gap: "0.4rem" }}>
                          <button onClick={() => copyUrl(file.url)} style={{ flex: 1, fontSize: "0.7rem", padding: "0.35rem", backgroundColor: "rgba(184,132,58,0.12)", border: "1px solid rgba(184,132,58,0.2)", borderRadius: "5px", color: "#B8843A", cursor: "pointer" }}>
                            {copied === file.url ? "✓" : "URL"}
                          </button>
                          <button onClick={() => deleteFile(file.key)} style={{ fontSize: "0.7rem", padding: "0.35rem 0.6rem", backgroundColor: "transparent", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "5px", color: "rgba(253,250,247,0.35)", cursor: "pointer" }}>✕</button>
                        </div>
                      </div>
                    </div>
                  );
