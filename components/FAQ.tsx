...
function Item({ q, a, i }: { q: string; a: string; i: number }) {
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ fontFamily: "var(--font-cormorant)", fontSize: "1.5rem", fontWeight: 500, color: "var(--cream)", lineHeight: 1.3 }}>
          {q}
        </span>
        <motion.span
          ...
          style={{ backgroundColor: open ? "var(--gold)" : "var(--hairline-strong)" }}
        >
          <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
            <line x1="6" y1="1" x2="6" y2="11" stroke={open ? "var(--black)" : "var(--cream-muted)"} strokeWidth="1.5" strokeLinecap="round" />
            <line x1="1" y1="6" x2="11" y2="6" stroke={open ? "var(--black)" : "var(--cream-muted)"} strokeWidth="1.5" strokeLinecap="round" />
          </svg>
        </motion.span>
      </button>
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            <p className="text-sm font-light leading-relaxed pb-5" style={{ color: "var(--cream-muted)", maxWidth: "600px" }}>
              {a}
            </p>
          </motion.div>
        )}
...
export default function FAQ() {
  return (
    <section id="faq" style={{ backgroundColor: "var(--black)", padding: "clamp(5rem,10vh,8rem) clamp(1.5rem,6vw,5rem)" }}>
      <div className="grid gap-16 grid-cols-1 lg:grid-cols-[1fr_1.6fr]" style={{ alignItems: "start" }}>
        <div className="lg:sticky lg:top-28">
          ...
          <AnimateIn delay={0.1}>
            <h2 style={{ fontFamily: "var(--font-cormorant)", fontSize: "clamp(1.75rem, 3.5vw, 2.5rem)", fontWeight: 300, lineHeight: 1.1, letterSpacing: 0, color: "var(--cream)" }}>
              Veelgestelde<br /><em style={{ color: "var(--cream-muted)" }}>vragen</em>
            </h2>
          </AnimateIn>
          <AnimateIn delay={0.2}>
            <p className="text-sm font-light leading-relaxed mt-5" style={{ color: "var(--cream-muted)", maxWidth: "300px" }}>
